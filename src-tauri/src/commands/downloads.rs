...
async fn run_db<F, T>(db: &Db, f: F) -> Result<T, AppError>
where
...
#[tauri::command]
pub async fn list_downloads(db: State<'_, Db>) -> Result<Vec<DownloadRecord>, AppError> {
    run_db(&db, |conn| {
        let mut stmt = conn
            .prepare(
...
#[tauri::command]
pub async fn delete_download(db: State<'_, Db>, id: String) -> Result<(), AppError> {
    run_db(&db, move |conn| {
...
#[tauri::command]
pub async fn clear_download_history(db: State<'_, Db>) -> Result<(), AppError> {
    run_db(&db, |conn| {

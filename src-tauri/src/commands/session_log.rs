...
#[tauri::command]
pub async fn list_sessions(state: tauri::State<'_, Db>) -> Result<Vec<SessionRecord>, String> {
    state
        .run(|conn| {
            let mut stmt = conn
                .prepare(
...
#[tauri::command]
pub async fn delete_session(state: tauri::State<'_, Db>, session_id: String) -> Result<(), String> {
    state
        .run(move |conn| {
            conn.execute(
